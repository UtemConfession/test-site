const translations={en:{nav_activities:"Activities",
...
}};let currentLang=localStorage.getItem("lang")||"en";function setLanguage(lang){currentLang=lang;document.documentElement.lang=lang;const t=translations[lang];if(!t)return;for(const[key,val]of Object.entries(t)){const el=document.getElementById(key);if(el){if(typeof val==='string'&&(val.includes('<')||val.includes('&'))){el.innerHTML=val;}else if(typeof val==='string'){el.textContent=val;}}}const tabNavMap={"activities-tab":{desktop:t.nav_activities,mobile:t.nav_activities},
...
"parcels-tab":{desktop:t.nav_parcels,mobile:t.mobile_nav_parcels}};const hrefNavMap={"calendar.html":{desktop:t.nav_calendar,mobile:t.mobile_nav_calendar},
...
"index.html#updates-tab":{desktop:t.nav_updates,mobile:t.mobile_nav_updates}};document.querySelectorAll(".sidebar .nav-menu .nav-item").forEach(item=>{const tab=item.getAttribute("data-tab");const href=item.getAttribute("href");if(tab&&tabNavMap[tab]){updateNodeText(item,tabNavMap[tab].desktop);}else if(href&&hrefNavMap[href]){updateNodeText(item,hrefNavMap[href].desktop);}});document.querySelectorAll(".mobile-bottom-nav .mobile-nav-btn, .mobile-drawer-grid .drawer-item-btn").forEach(btn=>{const tab=btn.getAttribute("data-tab");const href=btn.getAttribute("href");const span=btn.querySelector("span");if(span){if(tab&&tabNavMap[tab]){span.textContent=tabNavMap[tab].mobile;}else if(href&&hrefNavMap[href]){span.textContent=hrefNavMap[href].mobile;}}});const desktopToggle=document.getElementById("desktopLangToggle");const mobileToggle=document.getElementById("mobileLangToggle")||document.getElementById("mobileLangToggleOld");const drawerToggle=document.getElementById("drawerLangToggle");const toggleLabel=lang==="en"?"BM":"EN";const langAria=lang==="en"?"Switch to Bahasa Melayu":"Tukar ke Bahasa Inggeris";[desktopToggle,mobileToggle,drawerToggle].forEach(btn=>{if(!btn)return;const span=btn.querySelector("span");if(span){span.textContent=toggleLabel;}else{btn.textContent=toggleLabel;}btn.setAttribute("aria-label",langAria);btn.setAttribute("title",langAria);});const drawerTitle=document.getElementById("drawerHeaderTitle");if(drawerTitle)drawerTitle.textContent=t.drawer_services||(lang==="en"?"Services":"Perkhidmatan");const themeModalHeading=document.getElementById("themeModalHeading");if(themeModalHeading)themeModalHeading.textContent=t.theme_modal_title||(lang==="en"?"Curated Themes":"Pilih Tema");const themeModalSub=document.querySelector(".theme-modal-sub");if(themeModalSub)themeModalSub.textContent=t.theme_modal_sub||(lang==="en"?"Choose your preferred aesthetic style":"Pilih tema visual kegemaran anda");const langSwitchLabel=document.getElementById("langSwitchLabel");if(langSwitchLabel)langSwitchLabel.textContent=t.lang_label;const confessionTitle=document.querySelector("#confession-tab .card-title");updateNodeText(confessionTitle,t.title_submit_confession);const labelSubmit=document.querySelector("#confession-tab .form-group .form-label");if(labelSubmit)labelSubmit.textContent=t.label_submit_confession;const confText=document.getElementById("confessionText");if(confText)confText.setAttribute("placeholder",t.placeholder_confession);const chCountLabel=document.getElementById("charCount");if(chCountLabel&&confText){const len=confText.value.length;chCountLabel.textContent=lang==="en"?`${len} / 10000 characters`:`${len} / 10000 aksara`;}const agreeLabel=document.querySelector("#confession-tab .checkbox-container");if(agreeLabel)updateNodeText(agreeLabel,t.checkbox_rules);const sBtn=document.getElementById("submitBtn");if(sBtn&&sBtn.textContent!=="Submitting..."&&sBtn.textContent!=="Menghantar..."){sBtn.textContent=t.btn_submit;}const gFormBtn=document.querySelector("#confession-tab .btn-gold-outline");if(gFormBtn)gFormBtn.textContent=t.btn_submit_google;const guidelinesTitle=document.querySelector("#confession-tab .card:nth-child(2) .card-title");updateNodeText(guidelinesTitle,t.title_guidelines);const guides=document.querySelectorAll("#confession-tab .guide-list li");if(guides.length>=11){guides[0].querySelector("strong").textContent=t.guide_anon_title;updateNodeText(guides[0],t.guide_anon_desc);guides[1].querySelector("strong").textContent=t.guide_respect_title;updateNodeText(guides[1],t.guide_respect_desc);guides[2].querySelector("strong").textContent=t.guide_spam_title;updateNodeText(guides[2],t.guide_spam_desc);guides[3].querySelector("strong").textContent=t.guide_image_title;updateNodeText(guides[3],t.guide_image_desc);guides[4].querySelector("strong").textContent=t.guide_mod_title;updateNodeText(guides[4],t.guide_mod_desc);guides[5].querySelector("strong").textContent=t.guide_links_title;updateNodeText(guides[5],t.guide_links_desc);guides[6].querySelector("strong").textContent=t.guide_impersonation_title;updateNodeText(guides[6],t.guide_impersonation_desc);guides[7].querySelector("strong").textContent=t.guide_threats_title;updateNodeText(guides[7],t.guide_threats_desc);guides[8].querySelector("strong").textContent=t.guide_accusations_title;updateNodeText(guides[8],t.guide_accusations_desc);guides[9].querySelector("strong").textContent=t.guide_connections_title;updateNodeText(guides[9],t.guide_connections_desc);guides[10].querySelector("strong").textContent=t.guide_think_title;updateNodeText(guides[10],t.guide_think_desc);}const gpaTitle=document.querySelector("#gpa-tab .card-title");updateNodeText(gpaTitle,t.title_gpa_calc);const deansListBadge=document.getElementById("deansListBadge");if(deansListBadge)deansListBadge.textContent=lang==="en"?"🏆 Dean's List (Anugerah Dekan) Eligible!":"🏆 Layak Anugerah Dekan (Dean's List)!";const firstClassBadge=document.getElementById("firstClassBadge");if(firstClassBadge)firstClassBadge.textContent=lang==="en"?"🎓 First Class (Kelas Pertama) Eligible!":"🎓 Layak Kelas Pertama (First Class)!";if(typeof window.calculateGpa==="function"){window.calculateGpa();}const gpaLabels=document.querySelectorAll("#gpa-tab .gpa-result-card .form-label");if(gpaLabels.length>=3){gpaLabels[0].textContent=t.label_sem_gpa;gpaLabels[1].textContent=t.label_sem_credits;gpaLabels[2].textContent=t.label_est_cgpa;}const gpaThs=document.querySelectorAll("#gpa-tab .gpa-table th");if(gpaThs.length>=4){gpaThs[0].textContent=t.th_course;gpaThs[1].textContent=t.th_credits;gpaThs[2].textContent=t.th_grade;gpaThs[3].textContent=t.th_actions;}const aRowBtn=document.getElementById("addRowBtn");if(aRowBtn)aRowBtn.textContent=t.btn_add_subject;const cGpaBtn=document.getElementById("clearGpaBtn");if(cGpaBtn)cGpaBtn.textContent=t.btn_clear_gpa;const pGpaBtn=document.getElementById("btnPrintGpa");if(pGpaBtn)pGpaBtn.textContent=t.btn_print_gpa;const priorLabels=document.querySelectorAll(".cgpa-calc-box .form-label");if(priorLabels.length>=2){priorLabels[0].textContent=t.label_prior_cgpa;priorLabels[1].textContent=t.label_prior_credits;}const titleLinks=document.querySelector("#links-tab .card-title");if(titleLinks)updateNodeText(titleLinks,t.title_important_links||"UTeM Wi-Fi & Links");const descImportantLinks=document.getElementById("descImportantLinks");if(descImportantLinks)descImportantLinks.textContent=t.desc_important_links||"Quick access to essential official campus portals, VPN clients, student council channels, and academic systems.";const wifiRadarTitle=document.getElementById("wifiRadarTitle");if(wifiRadarTitle&&t.wifiRadarTitle)wifiRadarTitle.textContent=t.wifiRadarTitle;const wifiRadarSub=document.getElementById("wifiRadarSub");if(wifiRadarSub&&t.wifiRadarSub)wifiRadarSub.textContent=t.wifiRadarSub;const btnRunTestText=document.getElementById("btnRunTestText");if(btnRunTestText&&t.btnRunTestText)btnRunTestText.textContent=t.btnRunTestText;const libproxyTitle=document.getElementById("libproxyTitle");if(libproxyTitle)libproxyTitle.textContent=t.card_libproxy_title;const libproxySub=document.getElementById("libproxySub");if(libproxySub)libproxySub.textContent=t.card_libproxy_sub;const libproxyDesc=document.getElementById("libproxyDesc");if(libproxyDesc)libproxyDesc.textContent=t.card_libproxy_desc;const libproxyBtn=document.getElementById("libproxyBtn");if(libproxyBtn)updateNodeText(libproxyBtn,t.card_libproxy_btn);const vpnTitle=document.getElementById("vpnTitle");if(vpnTitle)vpnTitle.textContent=t.card_vpn_title;const vpnSub=document.getElementById("vpnSub");if(vpnSub)vpnSub.textContent=t.card_vpn_sub;const vpnDesc=document.getElementById("vpnDesc");if(vpnDesc)vpnDesc.textContent=t.card_vpn_desc;const vpnBtn=document.getElementById("vpnBtn");if(vpnBtn)updateNodeText(vpnBtn,t.card_vpn_btn);const staffDirTitle=document.getElementById("staffDirTitle");if(staffDirTitle)staffDirTitle.textContent=t.card_staff_title;const staffDirSub=document.getElementById("staffDirSub");if(staffDirSub)staffDirSub.textContent=t.card_staff_sub;const staffDirDesc=document.getElementById("staffDirDesc");if(staffDirDesc)staffDirDesc.textContent=t.card_staff_desc;const staffDirBtn=document.getElementById("staffDirBtn");if(staffDirBtn)updateNodeText(staffDirBtn,t.card_staff_btn);const mppTitle=document.getElementById("mppTitle");if(mppTitle)mppTitle.textContent=t.card_mpp_title;const mppSub=document.getElementById("mppSub");if(mppSub)mppSub.textContent=t.card_mpp_sub;const mppDesc=document.getElementById("mppDesc");if(mppDesc)mppDesc.textContent=t.card_mpp_desc;const mppBtn=document.getElementById("mppBtn");if(mppBtn)updateNodeText(mppBtn,t.card_mpp_btn);const aduanTitle=document.getElementById("aduanTitle");if(aduanTitle)aduanTitle.textContent=t.card_aduan_title;const aduanSub=document.getElementById("aduanSub");if(aduanSub)aduanSub.textContent=t.card_aduan_sub;const aduanDesc=document.getElementById("aduanDesc");if(aduanDesc)aduanDesc.textContent=t.card_aduan_desc;const aduanBtn=document.getElementById("aduanBtn");if(aduanBtn)updateNodeText(aduanBtn,t.card_aduan_btn);const konvoTitle=document.getElementById("konvoTitle");if(konvoTitle)konvoTitle.textContent=t.card_konvo_title;const konvoSub=document.getElementById("konvoSub");if(konvoSub)konvoSub.textContent=t.card_konvo_sub;const konvoDesc=document.getElementById("konvoDesc");if(konvoDesc)konvoDesc.textContent=t.card_konvo_desc;const konvoBtn=document.getElementById("konvoBtn");if(konvoBtn)updateNodeText(konvoBtn,t.card_konvo_btn);const titleMarketplace=document.getElementById("titleMarketplace");if(titleMarketplace)updateNodeText(titleMarketplace,lang==="en"?"UTeM Marketplace & Student Services":"Pasar & Perkhidmatan Pelajar UTeM");const descMarketplace=document.getElementById("descMarketplace");if(descMarketplace)descMarketplace.textContent=lang==="en"
...
:"Platform sehenti kampus untuk pelajar UTeM membeli & menjual barangan terpakai, buku teks, gajet, dan menawarkan perkhidmatan seperti Grab, Personal Shopper, dan percetakan.";const badgeMarketplace=document.getElementById("badgeMarketplace");if(badgeMarketplace)badgeMarketplace.textContent=lang==="en"?"Student Deals & Services Hub":"Pusat Barangan & Perkhidmatan Pelajar";const titleMarketItems=document.getElementById("titleMarketItems");if(titleMarketItems)titleMarketItems.textContent=lang==="en"?"🛒 Pre-loved Items & Marketplace":"🛒 Barangan Terpakai & Pasar Pelajar";const subMarketItems=document.getElementById("subMarketItems");if(subMarketItems)subMarketItems.textContent=lang==="en"?"Buy & Sell Student Essentials":"Beli & Jual Keperluan Pelajar";const descMarketItems=document.getElementById("descMarketItems");if(descMarketItems)descMarketItems.textContent=lang==="en"
...
:"Beli & jual keperluan pelajar secara terus dengan rakan pelajar UTeM. Dari buku rujukan dan bahan belajar kepada barangan elektronik dan keperluan kampus.";const titleMarketServices=document.getElementById("titleMarketServices");if(titleMarketServices)titleMarketServices.textContent=lang==="en"?"🚘 Student Services Directory":"🚘 Direktori Perkhidmatan Pelajar";const subMarketServices=document.getElementById("subMarketServices");if(subMarketServices)subMarketServices.textContent=lang==="en"?"Campus Runners & Services":"Pelari Kampus & Perkhidmatan";const descMarketServices=document.getElementById("descMarketServices");if(descMarketServices)descMarketServices.textContent=lang==="en"
...
:"Temui perkhidmatan pelajar yang dipercayai di sekitar kampus, termasuk pemandu Grab kampus, pembeli peribadi (personal shopper), pelari cetakan (print runners), dan pembantu projek.";const sellerNoticeMarketplace=document.getElementById("sellerNoticeMarketplace");if(sellerNoticeMarketplace)sellerNoticeMarketplace.innerHTML=lang==="en"
...
:'Ada barang nak letgo atau buat servis student? 🚀 Iklankan di sini! E-melkan butiran korang terus ke admin di <a href="mailto:utemconfessionpromax@duck.com" style="color: #facc15; text-decoration: underline; font-weight: 700;">utemconfessionpromax@duck.com</a>';const titleCampusBus=document.getElementById("title_campus_bus");if(titleCampusBus)titleCampusBus.textContent=t.title_bus_transit||"UTeM Bus Timetable";const labelRadarTitle=document.getElementById("labelRadarTitle");if(labelRadarTitle)labelRadarTitle.textContent=lang==="en"?"UTeM Bus Route":"Laluan Bas UTeM";document.querySelectorAll(".bus-route-pill").forEach(pill=>{const routeKey=pill.getAttribute("data-route");if(routeKey==="kt")pill.textContent="KT (Ayer Keroh)";else if(routeKey==="ki")pill.textContent="KI (Induk)";else if(routeKey==="ep")pill.textContent="EP (Off-Campus)";else if(routeKey==="ftmk_regular")pill.textContent="KI ⇄ KT";else if(routeKey==="satria_regular")pill.textContent="Satria ⇄ Fakulti";else if(routeKey==="lestari_regular")pill.textContent="Lestari ⇄ Fakulti";});const busThs=document.querySelectorAll("#bus-tab .bus-schedule-table th, #bus-tab .gpa-table th");if(busThs.length>=3){busThs[0].textContent=t.th_departure||"Departure";busThs[1].textContent=t.th_day_type||"Days";busThs[2].textContent=t.th_status||"Status";}const busNote=document.querySelector(".bus-next-alert p");if(busNote)busNote.textContent=t.label_note;const departureLabel=document.querySelector(".bus-next-alert span.form-label");if(departureLabel)departureLabel.textContent=t.label_next_departure;const titlePublicBus=document.getElementById("titlePublicBus");if(titlePublicBus)titlePublicBus.textContent=t.titlePublicBus||t.title_public_bus||"Pink Bus M10A Official Timetable";const m10aTopRouteLink=document.getElementById("m10aTopRouteLink");if(m10aTopRouteLink&&t.m10aTopRouteLink)m10aTopRouteLink.innerHTML=t.m10aTopRouteLink;const m10aNoticeTitle=document.getElementById("m10aNoticeTitle");if(m10aNoticeTitle&&t.m10aNoticeTitle)m10aNoticeTitle.innerHTML=t.m10aNoticeTitle;const m10aNoticeBody=document.getElementById("m10aNoticeBody");if(m10aNoticeBody&&t.m10aNoticeBody)m10aNoticeBody.innerHTML=t.m10aNoticeBody;const labelRouteTimeline=document.getElementById("labelRouteTimeline");if(labelRouteTimeline)labelRouteTimeline.innerHTML=t.labelRouteTimeline||t.label_route_timeline||"M10A Route Stops &amp; Key Transit Hubs";const labelWeekend1=document.getElementById("labelWeekend1");if(labelWeekend1)labelWeekend1.textContent=t.labelWeekend1||t.label_weekend||"Fri-Sun Extension";const labelWeekend2=document.getElementById("labelWeekend2");if(labelWeekend2)labelWeekend2.textContent=t.labelWeekend2||t.label_weekend||"Fri-Sun Direct Campus Entry";const m10aDir1Title=document.getElementById("m10aDir1Title");if(m10aDir1Title&&t.m10aDir1Title)m10aDir1Title.innerHTML=t.m10aDir1Title;const m10aDir1Desc=document.getElementById("m10aDir1Desc");if(m10aDir1Desc&&t.m10aDir1Desc)m10aDir1Desc.innerHTML=t.m10aDir1Desc;const m10aDir1FirstLast=document.getElementById("m10aDir1FirstLast");if(m10aDir1FirstLast&&t.m10aDir1FirstLast)m10aDir1FirstLast.innerHTML=t.m10aDir1FirstLast;const m10aDir2Title=document.getElementById("m10aDir2Title");if(m10aDir2Title&&t.m10aDir2Title)m10aDir2Title.innerHTML=t.m10aDir2Title;const m10aDir2Desc=document.getElementById("m10aDir2Desc");if(m10aDir2Desc&&t.m10aDir2Desc)m10aDir2Desc.innerHTML=t.m10aDir2Desc;const m10aDir2FirstLast=document.getElementById("m10aDir2FirstLast");if(m10aDir2FirstLast&&t.m10aDir2FirstLast)m10aDir2FirstLast.innerHTML=t.m10aDir2FirstLast;const labelM10ADetails=document.getElementById("labelM10ADetails");if(labelM10ADetails)labelM10ADetails.innerHTML=t.labelM10ADetails||t.label_m10a_details||"&#128203; BAS.MY M10A Operations &amp; Transit Guidelines:";const m10aOp1Title=document.getElementById("m10aOp1Title");if(m10aOp1Title&&t.m10aOp1Title)m10aOp1Title.innerHTML=t.m10aOp1Title;const m10aOp1Desc=document.getElementById("m10aOp1Desc");if(m10aOp1Desc&&t.m10aOp1Desc)m10aOp1Desc.innerHTML=t.m10aOp1Desc;const m10aOp2Title=document.getElementById("m10aOp2Title");if(m10aOp2Title&&t.m10aOp2Title)m10aOp2Title.innerHTML=t.m10aOp2Title;const m10aOp2Desc=document.getElementById("m10aOp2Desc");if(m10aOp2Desc&&t.m10aOp2Desc)m10aOp2Desc.innerHTML=t.m10aOp2Desc;const m10aOp3Title=document.getElementById("m10aOp3Title");if(m10aOp3Title&&t.m10aOp3Title)m10aOp3Title.innerHTML=t.m10aOp3Title;const m10aOp3Desc=document.getElementById("m10aOp3Desc");if(m10aOp3Desc&&t.m10aOp3Desc)m10aOp3Desc.innerHTML=t.m10aOp3Desc;const m10aOp4Title=document.getElementById("m10aOp4Title");if(m10aOp4Title&&t.m10aOp4Title)m10aOp4Title.innerHTML=t.m10aOp4Title;const m10aOp4Desc=document.getElementById("m10aOp4Desc");if(m10aOp4Desc&&t.m10aOp4Desc)m10aOp4Desc.innerHTML=t.m10aOp4Desc;const m10aRefLabel=document.getElementById("m10aRefLabel");if(m10aRefLabel&&t.m10aRefLabel)m10aRefLabel.innerHTML=t.m10aRefLabel;const m10aRefBasMyTxt=document.getElementById("m10aRefBasMyTxt");if(m10aRefBasMyTxt&&t.m10aRefBasMyTxt)m10aRefBasMyTxt.innerHTML=t.m10aRefBasMyTxt;const m10aRefRouteTxt=document.getElementById("m10aRefRouteTxt");if(m10aRefRouteTxt&&t.m10aRefRouteTxt)m10aRefRouteTxt.innerHTML=t.m10aRefRouteTxt;const m10aRefMapTxt=document.getElementById("m10aRefMapTxt");if(m10aRefMapTxt&&t.m10aRefMapTxt)m10aRefMapTxt.innerHTML=t.m10aRefMapTxt;const m10aRefCausewayTxt=document.getElementById("m10aRefCausewayTxt");if(m10aRefCausewayTxt&&t.m10aRefCausewayTxt)m10aRefCausewayTxt.innerHTML=t.m10aRefCausewayTxt;const titleRouteLookup=document.getElementById("titleRouteLookup");if(titleRouteLookup)titleRouteLookup.textContent=t.title_route_lookup;const descRouteLookup=document.getElementById("descRouteLookup");if(descRouteLookup)descRouteLookup.textContent=t.desc_route_lookup;const labelSelectDest=document.getElementById("labelSelectDest");if(labelSelectDest)labelSelectDest.textContent=t.label_select_dest;const optSelectDest=document.getElementById("optSelectDest");if(optSelectDest)optSelectDest.textContent=t.opt_select_dest;const optDestTampin=document.getElementById("optDestTampin");if(optDestTampin)optDestTampin.textContent=t.opt_dest_tampin;const optDestJasin=document.getElementById("optDestJasin");if(optDestJasin)optDestJasin.textContent=t.opt_dest_jasin;const optDestKlebang=document.getElementById("optDestKlebang");if(optDestKlebang)optDestKlebang.textContent=t.opt_dest_klebang;const optDestPaya=document.getElementById("optDestPaya");if(optDestPaya)optDestPaya.textContent=t.opt_dest_paya;const busSourcesLabel=document.getElementById("busSourcesLabel");if(busSourcesLabel)busSourcesLabel.textContent=t.bus_sources_label;const destSelect=document.getElementById("melakaDestSelect");const resBox=document.getElementById("lookupResultBox");if(destSelect&&resBox&&destSelect.value){resBox.innerHTML=lookupDetails[destSelect.value][lang];}const calTitle=document.querySelector("#calendar-tab .card-title");updateNodeText(calTitle,t.title_calendar);const calBtns=document.querySelectorAll(".cal-filter-btn");if(calBtns.length>=5){calBtns[0].textContent=t.btn_cal_all;calBtns[1].textContent=t.btn_cal_academic;calBtns[2].textContent=t.btn_cal_exam;calBtns[3].textContent=t.btn_cal_break;calBtns[4].textContent=t.btn_cal_holiday;}const calSearchInput=document.getElementById("calendarSearch");if(calSearchInput)calSearchInput.setAttribute("placeholder",t.placeholder_search_cal);const calendarSourcesLabel=document.getElementById("calendarSourcesLabel");if(calendarSourcesLabel)calendarSourcesLabel.textContent=t.calendar_sources_label;const calendarLinkLabel=document.getElementById("calendarLinkLabel");if(calendarLinkLabel)updateNodeText(calendarLinkLabel,t.calendar_link_label);const titlePastExamsTab=document.getElementById("titlePastExamsTab");if(titlePastExamsTab)titlePastExamsTab.textContent=t.title_past_exams_tab;const descPastExamsTab=document.getElementById("descPastExamsTab");if(descPastExamsTab)descPastExamsTab.textContent=t.desc_past_exams_tab;const labelPastExamsLib=document.getElementById("labelPastExamsLib");if(labelPastExamsLib)labelPastExamsLib.textContent=t.label_past_exams_lib;const btnPastExamsLink=document.getElementById("btnPastExamsLink");if(btnPastExamsLink)updateNodeText(btnPastExamsLink,t.btn_past_exams_link);const titleHealthTab=document.getElementById("titleHealthTab");if(titleHealthTab)titleHealthTab.textContent=t.title_health_tab;const descHealthTab=document.getElementById("descHealthTab");if(descHealthTab)descHealthTab.textContent=t.desc_health_tab;const labelHealthHours=document.getElementById("labelHealthHours");if(labelHealthHours)labelHealthHours.textContent=t.label_health_hours;const labelHealthMainInduk=document.getElementById("labelHealthMainInduk");if(labelHealthMainInduk)labelHealthMainInduk.textContent=t.label_health_main_induk;const labelHoursIndukAcad=document.getElementById("labelHoursIndukAcad");if(labelHoursIndukAcad)labelHoursIndukAcad.textContent=t.label_hours_induk_acad;const labelHoursIndukBreak=document.getElementById("labelHoursIndukBreak");if(labelHoursIndukBreak)labelHoursIndukBreak.textContent=t.label_hours_induk_break;const labelHealthTechAyer=document.getElementById("labelHealthTechAyer");if(labelHealthTechAyer)labelHealthTechAyer.textContent=t.label_health_tech_ayer;const labelHoursTech=document.getElementById("labelHoursTech");if(labelHoursTech)labelHoursTech.textContent=t.label_hours_tech;const labelHealthBreaks=document.getElementById("labelHealthBreaks");if(labelHealthBreaks)labelHealthBreaks.textContent=t.label_health_breaks;const labelHoursBreakMonThu=document.getElementById("labelHoursBreakMonThu");if(labelHoursBreakMonThu)labelHoursBreakMonThu.textContent=t.label_hours_break_monthu;const labelHoursBreakFri=document.getElementById("labelHoursBreakFri");if(labelHoursBreakFri)labelHoursBreakFri.textContent=t.label_hours_break_fri;const labelHealthClosed=document.getElementById("labelHealthClosed");if(labelHealthClosed)labelHealthClosed.textContent=t.label_health_closed;const labelHealthServices=document.getElementById("labelHealthServices");if(labelHealthServices)labelHealthServices.textContent=t.label_health_services;const labelDentalApp=document.getElementById("labelDentalApp");if(labelDentalApp)labelDentalApp.textContent=t.label_dental_app;const btnDentalLink=document.getElementById("btnDentalLink");if(btnDentalLink)updateNodeText(btnDentalLink,t.btn_dental_link);const labelEmergKit=document.getElementById("labelEmergKit");if(labelEmergKit)labelEmergKit.textContent=t.label_emerg_kit;const btnEmergLink=document.getElementById("btnEmergLink");if(btnEmergLink)updateNodeText(btnEmergLink,t.btn_emerg_link);const labelHealthPolicies=document.getElementById("labelHealthPolicies");if(labelHealthPolicies)labelHealthPolicies.textContent=t.label_health_policies;const labelHealthExtMc=document.getElementById("labelHealthExtMc");if(labelHealthExtMc)labelHealthExtMc.textContent=t.label_health_ext_mc;const descHealthExtMc=document.getElementById("descHealthExtMc");if(descHealthExtMc)descHealthExtMc.textContent=t.desc_health_ext_mc;const labelHealthExamAbsence=document.getElementById("labelHealthExamAbsence");if(labelHealthExamAbsence)labelHealthExamAbsence.textContent=t.label_health_exam_absence;const descHealthExamAbsence=document.getElementById("descHealthExamAbsence");if(descHealthExamAbsence)descHealthExamAbsence.textContent=t.desc_health_exam_absence;const labelHealthCoverage=document.getElementById("labelHealthCoverage");if(labelHealthCoverage)labelHealthCoverage.textContent=t.label_health_coverage;const descHealthCoverage=document.getElementById("descHealthCoverage");if(descHealthCoverage)descHealthCoverage.textContent=t.desc_health_coverage;const labelHealthEmergencyTitle=document.getElementById("labelHealthEmergencyTitle");if(labelHealthEmergencyTitle&&t.labelHealthEmergencyTitle)labelHealthEmergencyTitle.textContent=t.labelHealthEmergencyTitle;const descHealthEmergency=document.getElementById("descHealthEmergency");if(descHealthEmergency&&t.descHealthEmergency)descHealthEmergency.textContent=t.descHealthEmergency;const labelHealthSecHotline1=document.getElementById("labelHealthSecHotline1");if(labelHealthSecHotline1&&t.labelHealthSecHotline1)labelHealthSecHotline1.textContent=t.labelHealthSecHotline1;const labelHealthSecHotline2=document.getElementById("labelHealthSecHotline2");if(labelHealthSecHotline2&&t.labelHealthSecHotline2)labelHealthSecHotline2.textContent=t.labelHealthSecHotline2;const labelHealthNatEmerg=document.getElementById("labelHealthNatEmerg");if(labelHealthNatEmerg&&t.labelHealthNatEmerg)labelHealthNatEmerg.textContent=t.labelHealthNatEmerg;const labelHealthContact=document.getElementById("labelHealthContact");if(labelHealthContact)labelHealthContact.textContent=t.label_health_contact;const labelHealthPhone=document.getElementById("labelHealthPhone");if(labelHealthPhone)labelHealthPhone.textContent=t.label_health_phone;const labelHealthEmail=document.getElementById("labelHealthEmail");if(labelHealthEmail)labelHealthEmail.textContent=t.label_health_email;const labelHealthAddress=document.getElementById("labelHealthAddress");if(labelHealthAddress)labelHealthAddress.textContent=t.label_health_address;const labelHealthRef=document.getElementById("labelHealthRef");if(labelHealthRef)labelHealthRef.textContent=t.label_health_ref;const titleLibTab=document.getElementById("titleLibTab");if(titleLibTab)titleLibTab.textContent=t.title_lib_tab;const descLibTab=document.getElementById("descLibTab");if(descLibTab)descLibTab.textContent=t.desc_lib_tab;const labelLibHours=document.getElementById("labelLibHours");if(labelLibHours)labelLibHours.textContent=t.label_lib_hours;const labelLibMainHours=document.getElementById("labelLibMainHours");if(labelLibMainHours)labelLibMainHours.textContent=t.label_lib_main_hours;const labelHoursLibMonThu=document.getElementById("labelHoursLibMonThu");if(labelHoursLibMonThu)labelHoursLibMonThu.textContent=t.label_hours_lib_monthu;const labelHoursLibFri=document.getElementById("labelHoursLibFri");if(labelHoursLibFri)labelHoursLibFri.textContent=t.label_hours_lib_fri;const labelLibWeekendHours=document.getElementById("labelLibWeekendHours");if(labelLibWeekendHours)labelLibWeekendHours.textContent=t.label_lib_weekend_hours;const labelHoursLibWeekend=document.getElementById("labelHoursLibWeekend");if(labelHoursLibWeekend)labelHoursLibWeekend.textContent=t.label_hours_lib_weekend;const labelLibBreakHours=document.getElementById("labelLibBreakHours");if(labelLibBreakHours)labelLibBreakHours.textContent=t.label_lib_break_hours;const labelHoursLibBreak=document.getElementById("labelHoursLibBreak");if(labelHoursLibBreak)labelHoursLibBreak.textContent=t.label_hours_lib_break;const labelLib24Hours=document.getElementById("labelLib24Hours");if(labelLib24Hours)labelLib24Hours.textContent=t.label_lib_24hours;const labelLibServices=document.getElementById("labelLibServices");if(labelLibServices)labelLibServices.textContent=t.label_lib_services;const labelLibPlaystation=document.getElementById("labelLibPlaystation");if(labelLibPlaystation)labelLibPlaystation.textContent=t.label_lib_playstation;const descLibPlaystation=document.getElementById("descLibPlaystation");if(descLibPlaystation)descLibPlaystation.textContent=t.desc_lib_playstation;const labelLibBoardgames=document.getElementById("labelLibBoardgames");if(labelLibBoardgames)labelLibBoardgames.textContent=t.label_lib_boardgames;const descLibBoardgames=document.getElementById("descLibBoardgames");if(descLibBoardgames)descLibBoardgames.textContent=t.desc_lib_boardgames;const labelLibBookingTitle=document.getElementById("labelLibBookingTitle");if(labelLibBookingTitle)labelLibBookingTitle.textContent=t.label_lib_booking_title;const descLibBookingText=document.getElementById("descLibBookingText");if(descLibBookingText)descLibBookingText.textContent=t.desc_lib_booking_text;const labelLibBorrowing=document.getElementById("labelLibBorrowing");if(labelLibBorrowing)labelLibBorrowing.textContent=t.label_lib_borrowing;const labelLibRenewals=document.getElementById("labelLibRenewals");if(labelLibRenewals)labelLibRenewals.textContent=t.label_lib_renewals;const descLibRenewals=document.getElementById("descLibRenewals");if(descLibRenewals)descLibRenewals.textContent=t.desc_lib_renewals;const labelLibEzproxy=document.getElementById("labelLibEzproxy");if(labelLibEzproxy)labelLibEzproxy.textContent=t.label_lib_ezproxy;const descLibEzproxy=document.getElementById("descLibEzproxy");if(descLibEzproxy)descLibEzproxy.textContent=t.desc_lib_ezproxy;const btnGoogle=document.getElementById("btnGoogleForm");if(btnGoogle)updateNodeText(btnGoogle,t.btn_submit_google);const btnChooseImage=document.getElementById("chooseImageBtn");if(btnChooseImage)updateNodeText(btnChooseImage,t.btn_choose_image);const imageCaptionInput=document.getElementById("imageCaptionInput");if(imageCaptionInput)imageCaptionInput.placeholder=t.placeholder_image_caption;const agreeImageLabel=document.getElementById("agreeImageRulesLabel");if(agreeImageLabel)updateNodeText(agreeImageLabel,t.checkbox_image_rules);const btnSubmitImage=document.getElementById("submitImageBtn");if(btnSubmitImage)updateNodeText(btnSubmitImage,t.btn_submit_image);const imgCharCountLabel=document.getElementById("imageCharCount");if(imgCharCountLabel&&imageCaptionInput){const len=imageCaptionInput.value.length;imgCharCountLabel.textContent=lang==="en"?`${len} / 500 characters`:`${len} / 500 aksara`;}const btnLibMail=document.getElementById("btnLibMail");if(btnLibMail)updateNodeText(btnLibMail,t.btn_lib_mail);const labelLibContact=document.getElementById("labelLibContact");if(labelLibContact)labelLibContact.textContent=t.label_lib_contact;const labelLibPhone=document.getElementById("labelLibPhone");if(labelLibPhone)labelLibPhone.textContent=t.label_lib_phone;const labelLibEmail=document.getElementById("labelLibEmail");if(labelLibEmail)labelLibEmail.textContent=t.label_lib_email;const labelLibAddress=document.getElementById("labelLibAddress");if(labelLibAddress)labelLibAddress.textContent=t.label_lib_address;const labelLibRef=document.getElementById("labelLibRef");if(labelLibRef)labelLibRef.textContent=t.label_lib_ref;const vehicleTitle=document.getElementById("vehicleTitle");if(vehicleTitle)vehicleTitle.textContent=t.card_vehicle_title;const vehicleSub=document.getElementById("vehicleSub");if(vehicleSub)vehicleSub.textContent=t.card_vehicle_sub;const vehicleDesc=document.getElementById("vehicleDesc");if(vehicleDesc)vehicleDesc.textContent=t.card_vehicle_desc;const vehicleBtn=document.getElementById("vehicleBtn");if(vehicleBtn)updateNodeText(vehicleBtn,t.card_vehicle_btn);const titleScholarships=document.getElementById("titleScholarships")||document.querySelector("#scholarships-tab .card-title");if(titleScholarships){if(titleScholarships.tagName==="SPAN"){titleScholarships.textContent=t.title_scholarships||t.titleScholarships||"Scholarship & Financial Aid";}else{updateNodeText(titleScholarships,t.title_scholarships||t.titleScholarships||"Scholarship & Financial Aid");}}const descScholarships=document.getElementById("descScholarships");if(descScholarships)descScholarships.textContent=t.desc_scholarships;const titleArchive=document.getElementById("archiveTitle");if(titleArchive)updateNodeText(titleArchive,t.title_archive||(currentLang==="ms"?"Suara Pelajar":"Student Voices"));const descArchive=document.getElementById("descArchive");if(descArchive)descArchive.textContent=t.desc_archive||(currentLang==="ms"?"Layan cerita santai, luahan pelajar, dan gosip panas kampus. Cari guna kata kunci atau tapis ikut kategori di bawah":"Catch up on campus tea, student rants, and relatable stories. Search by keywords or filter by category to dive in!");const noteArchiveText=document.getElementById("noteArchiveText");if(noteArchiveText){noteArchiveText.innerHTML=t.note_archive||(currentLang==="ms"
?"<strong>Nota Komuniti:</strong> Hantaran dan luahan di Suara Pelajar dikongsi semata-mata untuk hiburan, perkongsian santai, dan pengalaman sesama pelajar. Ia tidak mencerminkan pendirian rasmi universiti dan tiada sebarang niat untuk menyakiti, menyasarkan, atau memfitnah mana-mana pihak. Hayati dengan minda terbuka dan marilah kekalkan budaya kampus yang positif dan saling menghormati!"
:"<strong>Community Note:</strong> Confessions and stories shared in Student Voices are contributed for entertainment, peer relatability, and lighthearted campus discussions. They do not represent official university views and carry no intent to target, harm, or defame any individual or organization. Read with an open mind and keep the campus culture kind and respectful!");}const archiveSearchInput=document.getElementById("archiveSearch");if(archiveSearchInput)archiveSearchInput.setAttribute("placeholder",t.placeholder_archive_search||(currentLang==="ms"?"🔍 Cari confession ikut kata kunci, tag, topik...":"🔍 Search confessions by keyword, tags, topic..."));const noticeArchiveTakedown=document.getElementById("noticeArchiveTakedown");if(noticeArchiveTakedown){noticeArchiveTakedown.innerHTML=`<strong>${currentLang === "ms" ? "Permohonan Pemadaman Kandungan:" : "Content Removal Request:"}</strong> ${currentLang === "ms" ? "Jika terdapat hantaran pengakuan yang melibatkan anda dan anda mahu ia dipadam daripada arkib, sila hubungi pasukan admin melalui e-mel atau Telegram." : "If a confession post involves you and you wish to have it removed from the archive, please contact the admin team via email or Telegram."}`;}const btnContactAdminRemoval=document.getElementById("btnContactAdminRemoval");if(btnContactAdminRemoval&&t.btn_contact_admin_removal){updateNodeText(btnContactAdminRemoval,t.btn_contact_admin_removal);}const navSupport=document.getElementById("navSupport");if(navSupport)navSupport.textContent=t.nav_support;const titleSupport=document.getElementById("titleSupport");if(titleSupport)updateNodeText(titleSupport,t.title_support);const descSupport=document.getElementById("descSupport");if(descSupport)descSupport.textContent=t.desc_support;const labelFreeSupport=document.getElementById("labelFreeSupport");if(labelFreeSupport)labelFreeSupport.textContent=t.label_free_support;const titlePartnerDeals=document.getElementById("titlePartnerDeals");if(titlePartnerDeals)titlePartnerDeals.textContent=t.title_partner_deals;const descPartnerDeals=document.getElementById("descPartnerDeals");if(descPartnerDeals)descPartnerDeals.textContent=t.desc_partner_deals;const btnPartnerDeals1=document.getElementById("btnPartnerDeals1");if(btnPartnerDeals1)btnPartnerDeals1.textContent=currentLang==="ms"?"Terokai Tawaran Rakan Kongsi & Hadiah Pelajar 🎁 ➔":"Explore Partner Offers & Student Deals 🎁 ➔";const btnPartnerDeals2=document.getElementById("btnPartnerDeals2");if(btnPartnerDeals2)btnPartnerDeals2.textContent=currentLang==="ms"?"Lawat Laman Rakan Kongsi untuk menyokong laman ini ❤️ ➔":"Visit Partner Site to Support UTeM Confessions ❤️ ➔";const activitiesTitle=document.getElementById("activitiesTitle");if(activitiesTitle)updateNodeText(activitiesTitle,lang==="ms"?"Teroka Melaka":"Explore Melaka");const descActivities=document.getElementById("descActivities");if(descActivities)descActivities.textContent=lang==="ms"?"Temui aktiviti menarik di sekitar Melaka, dari tapak bersejarah hingga kawasan alam semula jadi yang tersembunyi.":"Discover things to do around Melaka, from historical sites to hidden nature spots.";const activitiesSearch=document.getElementById("activitiesSearch");if(activitiesSearch)activitiesSearch.placeholder=lang==="ms"?"🔍 Cari tempat, lokasi...":"🔍 Search attractions, location...";const activitiesTypeSelect=document.getElementById("activitiesTypeSelect");if(activitiesTypeSelect){activitiesTypeSelect.options[0].text=lang==="ms"?"🌍 Semua Jenis":"🌍 All Types";activitiesTypeSelect.options[1].text=lang==="ms"?"🏠 Tertutup":"🏠 Indoor";activitiesTypeSelect.options[2].text=lang==="ms"?"☀️ Luar":"☀️ Outdoor";}const btnPartnerDeals3=document.getElementById("btnPartnerDeals3");if(btnPartnerDeals3)btnPartnerDeals3.textContent=currentLang==="ms"?"Terokai Tawaran Tajaan Luar 🌐 ➔":"Explore External Sponsored Offers 🌐 ➔";const labelCoffeeSupport=document.getElementById("labelCoffeeSupport");if(labelCoffeeSupport)labelCoffeeSupport.textContent=t.label_coffee_support;const titleCoffee=document.getElementById("titleCoffee");if(titleCoffee)titleCoffee.textContent=t.title_coffee;const descCoffee=document.getElementById("descCoffee");if(descCoffee)descCoffee.textContent=t.desc_coffee;const noticeKofiCurrency=document.getElementById("noticeKofiCurrency");if(noticeKofiCurrency)noticeKofiCurrency.textContent=t.notice_kofi_currency;const btnKofiSupport=document.getElementById("btnKofiSupport");if(btnKofiSupport)btnKofiSupport.textContent=t.btn_kofi_support||"Buy Us a Coffee on Ko-fi ☕ ➔";const btnContactAdminTip=document.getElementById("btnContactAdminTip");if(btnContactAdminTip)btnContactAdminTip.textContent=t.btn_contact_admin_tip;const labelTransparencyTitle=document.getElementById("labelTransparencyTitle");if(labelTransparencyTitle)labelTransparencyTitle.textContent=t.label_transparency_title;const descTransparency=document.getElementById("descTransparency");if(descTransparency)descTransparency.textContent=t.desc_transparency;const titlePsmToolkit=document.getElementById("titlePsmToolkit");if(titlePsmToolkit){titlePsmToolkit.textContent=currentLang==="ms"
?"Kit Panduan Format Tesis & PSM (Projek Sarjana Muda) UTeM"
:"UTeM PSM (Projek Sarjana Muda) & Thesis Guidelines Toolkit";}const subtitlePsmToolkit=document.getElementById("subtitlePsmToolkit");if(subtitlePsmToolkit){subtitlePsmToolkit.textContent=currentLang==="ms"
?"Piawaian format tesis rasmi, templat Word/LaTeX, kriteria Turnitin & penghantaran e-Prints."
:"Official thesis formatting standards, Word/LaTeX templates, Turnitin criteria & e-Prints submission.";}const labelTurnitinHeading=document.getElementById("labelTurnitinHeading");if(labelTurnitinHeading){labelTurnitinHeading.textContent=currentLang==="ms"
?"Indeks Kesamaan Turnitin & Penghantaran Akhir Repositori (e-Prints)"
:"Turnitin Similarity Index & Final Repository Submission (e-Prints)";}const descTurnitinText=document.getElementById("descTurnitinText");if(descTurnitinText){descTurnitinText.innerHTML=currentLang==="ms"
...
:"UTeM faculties mandate a similarity index below <strong>20% &ndash; 25%</strong> (excluding bibliography and quotations). Before hard-cover binding or final e-submission, verify your report through your supervisor's Turnitin classroom. Approved final theses must be uploaded to the official <strong>UTeM e-Prints Institutional Repository</strong>.";}const labelWifiAssistant=document.getElementById("labelWifiAssistant");if(labelWifiAssistant){labelWifiAssistant.textContent=currentLang==="ms"
?"🛠️ Konfigurator Wi-Fi Peranti & Pintasan Pintar"
:"🛠️ Device Enterprise Configurator & Smart Bypass";}const subWifiAssistant=document.getElementById("subWifiAssistant");if(subWifiAssistant){subWifiAssistant.textContent=currentLang==="ms"
?"Pilih sistem operasi anda untuk tetapan terperinci atau muat turun profil Apple 1-ketik:"
:"Select your OS for exact drop-down parameters or download a 1-tap Apple profile:";}const footerDiv=document.querySelector("footer div");if(footerDiv){footerDiv.innerHTML=`<strong>UTeM Confessions Pro Max</strong> &copy; ${t.footer_text}`;}const footerLinksMap={"index.html":t.footer_home||"Home",
...
};document.querySelectorAll(".footer-links a").forEach(link=>{const href=link.getAttribute("href");if(href&&footerLinksMap[href]){link.textContent=footerLinksMap[href];}});try{if(typeof updateBusScheduleDisplay==='function')updateBusScheduleDisplay();if(typeof updateM10ANextDeparture==='function')updateM10ANextDeparture();if(typeof renderMilestonePill==='function')renderMilestonePill();else if(typeof window.renderMilestonePill==='function')window.renderMilestonePill();if(typeof renderAcademicWeekTracker==='function')renderAcademicWeekTracker();else if(typeof window.renderAcademicWeekTracker==='function')window.renderAcademicWeekTracker();if(typeof renderCalendarEvents==='function'){const activeCalBtn=document.querySelector(".cal-filter-btn.active");const category=activeCalBtn?activeCalBtn.getAttribute("data-category"):'all';const calSearch=document.getElementById("calendarSearch");renderCalendarEvents(category,calSearch?calSearch.value:'');}try{applyScholarshipTranslations();}catch(err){console.warn('Scholarship translation error:',err);}try{if(typeof refreshActivitiesTranslations==='function')refreshActivitiesTranslations();}catch(err){console.warn('Activities translation error:',err);}if(typeof renderMilestonePill==='function')renderMilestonePill();else if(typeof window.renderMilestonePill==='function')window.renderMilestonePill();if(typeof initExamSeasonPill==='function')initExamSeasonPill();else if(typeof window.initExamSeasonPill==='function')window.initExamSeasonPill();if(typeof initLibraryHoursPill==='function')initLibraryHoursPill();else if(typeof window.initLibraryHoursPill==='function')window.initLibraryHoursPill();if(typeof initPkuHoursPill==='function')initPkuHoursPill();else if(typeof window.initPkuHoursPill==='function')window.initPkuHoursPill();if(typeof updateUnitMelStatus==='function')updateUnitMelStatus();else if(typeof window.updateUnitMelStatus==='function')window.updateUnitMelStatus();}catch(e){console.warn("Dynamic component translation warning:",e);}}function toggleLanguage(){currentLang=currentLang==="en"?"ms":"en";localStorage.setItem("lang",currentLang);setLanguage(currentLang);}const scholarshipDict={"Bantuan Kewangan Kecemasan (UTeM Pusat Islam)":{ms:"Bantuan Kewangan Kecemasan (UTeM Pusat Islam)"},
...
"Target:":{ms:"Sasaran:"},
"Sasaran:":{en:"Target:"}};function applyScholarshipTranslations(){const cards=document.querySelectorAll('.scholarship-card, .state-panel, #scholarships-tab .scholarship-card, #stateScholarshipsResult .state-panel');cards.forEach(card=>{const titleEl=card.querySelector('h3, h4');const descEl=card.querySelector('p');const targetEl=card.querySelector('strong');[titleEl,descEl,targetEl].forEach(el=>{if(!el)return;if(!el.hasAttribute('data-orig-text')){el.setAttribute('data-orig-text',el.textContent.replace(/\s+/g,' ').trim());}const orig=el.getAttribute('data-orig-text');const map=scholarshipDict[orig];if(map){if(currentLang==='ms'&&map.ms){el.textContent=map.ms;}else if(currentLang==='en'&&map.en){el.textContent=map.en;}else{el.textContent=orig;}}});});}