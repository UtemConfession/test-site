...
const translations = {
    en: {
        ...
        hero_hub_title: "UTeM Confessions Hub",
        hero_queue_open: "Queue Open",
        hero_subtitle: "Your safe, 100% anonymous space for campus confessions, rants & peer stories.",
        hero_btn_telegram: "Telegram Channel",
        hero_stat_confessions: "<strong>5,000+</strong> Confessions Published",
        hero_privacy_shield: "100% Anonymous & Unlinked",
    ...
    ms: {
        ...
        hero_hub_title: "Hab Luahan UTeM",
        hero_queue_open: "Giliran Dibuka",
        hero_subtitle: "Ruang selamat & 100% tanpa nama untuk luahan, kehidupan kampus & suara mahasiswa.",
        hero_btn_telegram: "Saluran Telegram",
        hero_stat_confessions: "<strong>5,000+</strong> Luahan Diterbitkan",
        hero_privacy_shield: "100% Tanpa Nama & Privasi",
        draft_saved: "Draf disimpan automatik",
...
function setLanguage(lang) {
    ...
    // 4. Confessions tab & Hero Banner
    const heroLiveStatus = document.getElementById("heroLiveStatusText");
    if (heroLiveStatus) heroLiveStatus.textContent = t.hero_queue_open || (lang === "ms" ? "Giliran Dibuka" : "Queue Open");

    const heroSubtitle = document.getElementById("heroGreetingSubtitle");
    if (heroSubtitle) heroSubtitle.textContent = t.hero_subtitle || (lang === "ms" ? "Ruang selamat & 100% tanpa nama untuk luahan, kehidupan kampus & suara mahasiswa." : "Your safe, 100% anonymous space for campus confessions, rants & peer stories.");

    const heroBtnTelegram = document.getElementById("heroBtnTelegramText");
    if (heroBtnTelegram) heroBtnTelegram.textContent = t.hero_btn_telegram || (lang === "ms" ? "Saluran Telegram" : "Telegram Channel");

    const heroStatConf = document.getElementById("heroStatConfessions");
    if (heroStatConf) heroStatConf.innerHTML = t.hero_stat_confessions || (lang === "ms" ? "<strong>5,000+</strong> Luahan Diterbitkan" : "<strong>5,000+</strong> Confessions Published");

    const heroPrivacy = document.getElementById("heroPrivacyText");
    if (heroPrivacy) heroPrivacy.textContent = t.hero_privacy_shield || (lang === "ms" ? "100% Tanpa Nama & Privasi" : "100% Anonymous & Unlinked");

    const draftStatusText = document.getElementById("draftStatusText");
    if (draftStatusText) draftStatusText.textContent = t.draft_saved || (lang === "ms" ? "Draf disimpan automatik" : "Draft auto-saved");

    const confessionTitle = document.querySelector("#confession-tab .card-title");
    if (confessionTitle) updateNodeText(confessionTitle, t.title_submit_confession);

    const titleSubmitConf = document.getElementById("titleSubmitConfession");
    if (titleSubmitConf) titleSubmitConf.textContent = t.title_submit_confession;
    ...
    const labelAgreeRules = document.getElementById("labelAgreeRules");
    if (labelAgreeRules) labelAgreeRules.textContent = t.checkbox_rules;

    const sBtn = document.getElementById("submitBtn");
    if (sBtn && sBtn.textContent !== "Submitting..." && sBtn.textContent !== "Menghantar...") {
        const btnSubmitText = document.getElementById("btnSubmitText");
        if (btnSubmitText) btnSubmitText.textContent = t.btn_submit;
        else updateNodeText(sBtn, t.btn_submit);
    }
    ...
    if (typeof window.updateHeroGreeting === "function") {
        window.updateHeroGreeting(lang);
    }

    const guidelinesTitle = document.querySelector("#confession-tab .card:nth-child(2) .card-title");
    if (guidelinesTitle) updateNodeText(guidelinesTitle, t.title_guidelines);
