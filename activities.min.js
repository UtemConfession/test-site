...
};const suitableArr=item.suitableFor?item.suitableFor.slice(0,3).map(s=>suitableTranslationMap[s]||s).join(", "):"";const catIcon=activitiesCategories.find(c=>c.id===item.categoryId)?.icon||'🏛️';const imgHTML=`
...
${lang==='ms'?'Laman Web Rasmi':'Official Website'}</a>`;
    }
